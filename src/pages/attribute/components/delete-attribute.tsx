...
export function DeleteAlertDialog({ data, openDialogDelete, onClose }: { data: any, openDialogDelete: boolean, onClose: () => void }) {
    ...
    const fetchDeleteAttribute = async () => {
        setLoading(true);
        try {
            const response = await fetchDeleteAsset(`atribut/${data.id}`);
            if (response) {
                toast({
                    title: "Success!",
                    description: "Data Atribut Berhasil Dihapus!",
                });
                ...
                toast({
                    variant: "destructive",
                    title: "Wrong",
                    description: "Data Atribut Gagal Dihapus",
                });
                setLoading(false)
                throw new Error("Data Atribut Gagal Dihapus!");
            }
        } catch (error) {
            toast({
                variant: "destructive",
    ...
    return (
      <AlertDialog open={openDialogDelete} onOpenChange={onClose}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Data Atribut?</AlertDialogTitle>
            <AlertDialogDescription>
