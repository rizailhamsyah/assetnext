...
import { fetchGetAsset, fetchPutAsset } from "@/service/api"
...
export function FormEditCategory({ data, openDialogEdit, onClose }: { data: any, openDialogEdit: boolean, onClose: () => void }) {
        ...
        if (data) {
            const newAttributes = data.kategoriAtribut.map((attribute: any) => {
                return {
                    ...attribute,
                    id: attribute.id,
                    value: attribute.atribut_id
                };
            });
            setAttributes(newAttributes);
            setIdCategory(data.id_kategori);
            setCategory(data.kategori);
            setGroup(data.kelompok_asset);
            setCompilator(data.kompilator_id);
            setBenefit(data.masa_manfaat_id);
            createFormik.setFieldValue("idCategory", idCategory);
            createFormik.setFieldValue("category", category);
    ...
    }, [data, dataCompilator, dataAttribut, dataBenefit]);
    ...
    const createFormik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                const response = await fetchPutAsset(`kategori/${data.id}`, formData);
                if(response){
    ...
    return (
        <Dialog open={openDialogEdit} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>#{data.id} Edit Kategori</DialogTitle>
                    <DialogDescription>
                ...
                <form onSubmit={createFormik.handleSubmit}>
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="grid w-full max-w-sm items-center gap-1.5">
                            ...
                            <Select name="group" onValueChange={onChangeGroup} value={createFormik.values.group.toString()} disabled={loading}>
                        ...
                        <div className="grid w-full max-w-sm items-center gap-1.5">
                            ...
                            <Select name="compilator" onValueChange={onChangeCompilator} value={createFormik.values.compilator.toString()} disabled={loading}>
                        ...
                        <div className="grid w-full max-w-sm items-center gap-1.5">
                            ...
                            <Select name="benefit" onValueChange={onChangeBenefit} value={createFormik.values.benefit.toString()} disabled={loading}>
                        ...
                        {attributes.map((attribute, index) => (
                            <div key={attribute.id} className="grid w-full max-w-sm items-center gap-1.5">
                                ...
                                <div className="grid grid-cols-12 gap-x-2">
                                    <Select name={`attribut-${attribute.id}`} required={true} onValueChange={(value) => handleChangeAttribute(attribute.id, value)} value={attribute.value.toString()} disabled={loading}>
